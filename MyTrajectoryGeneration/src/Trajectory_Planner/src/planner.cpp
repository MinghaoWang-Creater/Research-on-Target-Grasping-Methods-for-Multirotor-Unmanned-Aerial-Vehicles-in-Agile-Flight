...
nav_msgs::Path planner::trajectory_path(void)
{
    ...
    for (int i = 0; i < time.size(); i++) 
    {
        ...
        for (double t = 0.0; t < time(i); t += 0.01) 
        {
            ...
            pt.pose.position.x = pos(0);  //取t时刻的pos值，并赋值给pt，
            pt.pose.position.y= pos(1);   //下一时刻pt就会发生变化，无法保留，
            pt.pose.position.z = pos(2);  //所以将pt打印出来后，只有两次的
            trajectory.poses.push_back(pt); //trajectory是一个向量，每个元素都是由pt赋值而来的，打印出来有是有所有时刻的信息
        }                                   //如果想要pt也包含所有时刻的信息，那么需要一个新的向量，类型为PoseStamped
    }
    // cout << "trajectory:" << trajectory << endl;
    // cout << "hahahahahahahahahahahahahahahahahahahahahahahahaha"<<endl;
    // cout << "PoseStamped" << pt << endl;
...
//         {
//             pos = getPosPoly(poly_coeff, i, t);
//             pt.pose.position.x.push_back(pos(0));  //取t时刻的pos值，并赋值给pt，
//             pt.pose.position.y.push_back(pos(1));   //下一时刻pt就会发生变化，无法保留，
//             pt.pose.position.z.push_back(pos(2));  //所以将pt打印出来后，只有两次的
//             // trajectory.poses.push_back(pt); //trajectory是一个向量，每个元素都是由pt赋值而来的，打印出来有是有所有时刻的信息
//         }                                   //如果想要pt也包含所有时刻的信息，那么需要一个新的向量，类型为PoseStamped
//     }
//     // cout << "trajectory:" << trajectory << endl;
...
void planner::tra_publish(void)
{
    ...
    // ros::Publisher tra_generation_pub = n.advertise<nav_msgs::Path>("/firefly/command/pose", 10); 
